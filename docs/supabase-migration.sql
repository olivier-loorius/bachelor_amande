-- 🔄 Migration vers Supabase - Amande Pâtisserie
-- Ce script adapte votre structure actuelle pour Supabase
...
-- Table des catégories (conservée)
...
-- Table des ingrédients (conservée)
...
-- Table des profils utilisateurs (nouvelle pour Supabase)
...
-- Table des compositions (adaptée pour Supabase)
...
BEGIN
  ...
  SET status = 'deleted', 
      deleted_at = NOW(),
      email_hash = hash_email((SELECT email FROM auth.users WHERE id = user_uuid))
  WHERE id = user_uuid;
...
-- Fonction pour mettre à jour le hash email automatiquement
CREATE OR REPLACE FUNCTION update_email_hash()
RETURNS TRIGGER AS $$
BEGIN
  NEW.email_hash = hash_email((SELECT email FROM auth.users WHERE id = NEW.id));
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- Trigger pour email_hash
CREATE TRIGGER update_user_profiles_email_hash
  BEFORE INSERT OR UPDATE ON user_profiles
  FOR EACH ROW EXECUTE FUNCTION update_email_hash();
...
-- Note: Les utilisateurs et compositions seront recréés via l'interface
-- car ils nécessitent des UUIDs et l'authentification Supabase
